...
from .serializers import PresentSerializer, CardSerializer
from .models import Present, Card
# Create your views here.l

class PresentAPIView(APIView) :
    # 모든 녹음 조회
...
class PresentDetailAPIView(APIView) :
    #로그인 여부 확인
...
class UserPresentAPIView(APIView) :
    # 모든 아바타 조회
    def get(self, request):
        if request.user.is_superuser == False:
            raise PermissionDenied("권한이 없습니다.")
...
class CardAPIView(APIView) :
    # 선물 카드 이미지 등록
    def get_object(self, card_pk, request):
        if request.user.is_superuser == False:
            raise PermissionDenied("권한이 없습니다.")
